		...
		<div class="table-wrapper">
			<div class="table-title">
				<div class="row">
					...
					<div class="col-sm-6">
						<a href="{{route('room.store')}}" class="btn btn-success"><i class="material-icons">&#xE147;</i> <span>Add New Room</span></a>
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover">
				...
				<tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="" class="btn btn-success" data-toggle="modal"><span>Details</span></a></td>
                        <td>
                            @if($room->status == 1)
                                Disponible
                            @else
                        ...
                        <td>
                            <a href="{{ route('room.edit', ['room' => $room->id]) }}" class="edit" data-toggle="modal" data-target="#editEmployeeModal{{ $room->id }}">
                ...
                <form method="POST" action="{{ route('room.update', ['room' => $room->id]) }}">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="lastname" value="{{ $room->lastname }}" required>
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <input type="text" class="form-control" name="username" value="{{ $room->username }}" required>
                        </div>
                        <div class="form-group">
                            <label>Room name</label>
                            <input type="text" class="form-control" name="birthday" value="{{ $room->birthday }}" required>
                        </div>
                        <div class="form-group">
                            <label>Price</label>
                            <input type="email" class="form-control" name="email" value="{{ $room->email }}" required>
                        </div>
                        <div class="form-group">
                            <label>Bed numbers</label>
                            <input type="number" class="form-control" name="email" value="{{ $room->email }}" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="role" required>
                                <option value="1" {{ $room->role == 1 ? 'selected' : '' }}>Disponible</option>
                                <option value="0" {{ $room->role == 0 ? 'selected' : '' }}>Non disponible</option>
                            </select>
                        </div>
                    </div>
    ...
    <div id="deleteEmployeeModal{{ $room->id }}" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <<form method="POST" action="{{ route('room.destroy', ['room' => $room->id]) }}">
                            @csrf
                            @method('DELETE')
